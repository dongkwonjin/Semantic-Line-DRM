...
class Train_Process_RNet(object):
    ...
    def training(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            pairwise = generate_line_pair_RNet(training_line=batch['training_data'],
    ...
    def validation(self):
        auc = self.test_process.run(self.DNet, self.RNet, mode='val')
        logger("Epoch %03d finished... AUC_10 : %5f\n"
               % (self.ckpt['epoch'], auc), self.logfile)
        self.val_result['AUC_10'] = save_model_max(self.ckpt, self.cfg.weight_dir,
                                                  self.val_result['AUC_10'], auc,
                                                  logger, self.logfile, 'AUC_10')
