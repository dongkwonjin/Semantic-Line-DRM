...
from networks.comparator import RNet, MNet
...
def load_RNet_for_test(cfg, dict_DB):
    if cfg.run_mode == 'test_paper':
        ...
        # select ckpt from pretrained_RNet_dir
        checkpoint = torch.load(cfg.pretrained_RNet_dir + '')
...
def load_MNet_for_test(cfg, dict_DB):
    if cfg.run_mode == 'test_paper':
        checkpoint = torch.load(cfg.paper_weight_dir + 'checkpoint_MNet_paper')
    else:
...
def load_MNet_for_train(cfg, dict_DB):
    model = MNet()
    model.cuda()
    ...
    if cfg.resume == False:
        checkpoint = torch.load(cfg.weight_dir + 'checkpoint_MNet_final')
        model.load_state_dict(checkpoint['model'], strict=False)
