...
def main_test(cfg, dict_DB):
    ...
    # test optoin
    test_process = Test_Process_DNet(cfg, dict_DB)
    test_process.run(dict_DB['DNet'])


def main_train(cfg, dict_DB):

    dict_DB['test_process'] = Test_Process_DNet(cfg, dict_DB)

    train_process = Train_Process_DNet(cfg, dict_DB)
    train_process.run()
