...
class Train_Process_DNet(object):
    ...
    def validation(self):
        auc_p, auc_r = self.test_process.run(self.DNet, mode='val')
        logger("Epoch %03d finished... AUC_P %5f / AUC_R %5f\n" % (self.ckpt['epoch'], auc_p, auc_r), self.logfile)
        self.val_result['AUC_P_upper_R_0.90'] = save_model_max_upper(self.ckpt, self.cfg.weight_dir,
                                                                     self.val_result['AUC_P_upper_R_0.90'], auc_p, auc_r, 0.90,
                                                                     logger, self.logfile, 'AUC_P_upper_R_0.90')
        self.val_result['AUC_P_upper_R_0.95'] = save_model_max_upper(self.ckpt, self.cfg.weight_dir,
                                                                     self.val_result['AUC_P_upper_R_0.95'], auc_p, auc_r, 0.95,
                                                                     logger, self.logfile, 'AUC_P_upper_R_0.95')
        self.val_result['AUC_P_upper_R_0.97'] = save_model_max_upper(self.ckpt, self.cfg.weight_dir,
                                                                     self.val_result['AUC_P_upper_R_0.97'], auc_p, auc_r, 0.97,
                                                                     logger, self.logfile, 'AUC_P_upper_R_0.97')
