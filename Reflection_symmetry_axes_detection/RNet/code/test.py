...
class Test_Process_DR(object):
    def __init__(self, cfg, dict_DB):
        ...
        self.candidates = load_pickle(self.cfg.test_dataset_dir +
                                      'data/detector/detector_test_candidates')
    ...
    def run(self, DNet, RNet, mode='test'):
        ...
        with torch.no_grad():
            ...
            for i, self.batch in enumerate(self.dataloader):  # load batch data
                ...
                pos_num = np.maximum(np.minimum(25, int(torch.sum(pos_check))), 1)
                out['pos'] = out['pts'][sorted[:pos_num]]
